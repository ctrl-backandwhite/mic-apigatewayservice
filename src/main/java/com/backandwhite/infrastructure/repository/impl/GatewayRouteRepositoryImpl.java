...
public class GatewayRouteRepositoryImpl implements GatewayRouteRepository {
     ...
     * - Si la ruta no existe → la inserta con enabled=true y createdAt=NOW().
     * - Si ya existe → actualiza uri, predicates, filters, order y rate-limit.
     *   Preserva enabled y createdAt para no pisar cambios manuales.
     */
    @Override
    public Mono<Void> upsert(GatewayRoute route) {
        ...
        return databaseClient.sql("""
                ...
                .bind("uri", entity.getUri())
                .bind("predicates", entity.getPredicates())
                .bind("filters", entity.getFilters())
                .bind("order", entity.getOrder())
                .bind("replenishRate", entity.getRateLimitReplenishRate() != null
                        ? entity.getRateLimitReplenishRate() : io.r2dbc.spi.Parameters.inOut(io.r2dbc.spi.R2dbcType.INTEGER))
                .bind("burstCapacity", entity.getRateLimitBurstCapacity() != null
                        ? entity.getRateLimitBurstCapacity() : io.r2dbc.spi.Parameters.inOut(io.r2dbc.spi.R2dbcType.INTEGER))
                .bind("requestedTokens", entity.getRateLimitRequestedTokens() != null
                        ? entity.getRateLimitRequestedTokens() : io.r2dbc.spi.Parameters.inOut(io.r2dbc.spi.R2dbcType.INTEGER))
                .fetch()
                .rowsUpdated()
                .then();
    }
